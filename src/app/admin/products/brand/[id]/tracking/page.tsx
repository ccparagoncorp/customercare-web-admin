...
import { ArrowLeft, Filter, Search, Calendar, User, FileText, RefreshCw, Package } from "lucide-react"
...
function BrandTrackingContent() {
  ...
  }, [brandId, filterAction, filterTable])
  ...
  }, [session, status, router, fetchLogs, brandId, brandLoading])
  ...
  const getTableDisplayName = (table: string) => {
    const tableNames: Record<string, string> = {
      ...
      'subkategori_produks': 'Subkategori',
      'produks': 'Produk',
      'detail_produks': 'Detail Produk',
      'knowledges': 'Knowledge',
      'detail_knowledges': 'Detail Knowledge',
      'jenis_detail_knowledges': 'Jenis Detail Knowledge',
      'produk_jenis_detail_knowledges': 'Produk Jenis Detail Knowledge',
      'sops': 'SOP',
      'jenis_sops': 'Jenis SOP',
      'detail_sops': 'Detail SOP',
      'quality_trainings': 'Quality Training',
      'jenis_quality_trainings': 'Jenis Quality Training',
      'detail_quality_trainings': 'Detail Quality Training',
      'subdetail_quality_trainings': 'Subdetail Quality Training',
    }
  ...
  // Group logs by change timestamp and sourceKey
  const groupedLogs = logs.reduce((acc, log) => {
    const key = `${log.sourceTable}-${log.sourceKey}-${log.changedAt}`
    if (!acc[key]) {
      acc[key] = {
  ...
  const filteredLogs = Object.values(groupedLogs).filter((group) => {
    ...
    if (filterTable !== 'ALL' && group.sourceTable !== filterTable) {
      return false
    }
    if (searchTerm) {
      ...
      return (
        ...
        getTableDisplayName(group.sourceTable).toLowerCase().includes(search) ||
  ...
  const uniqueTables = Array.from(new Set(logs.map(log => log.sourceTable)))
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h1 className="text-3xl font-bold mb-2">Tracking: {brand.name}</h1>
                  <p className="text-blue-100 text-lg">Riwayat perubahan data terkait brand ini</p>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Cari berdasarkan ID, tabel, field, atau nilai..."
                  value={searchTerm}
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-gray-500">
              ...
              <p className="text-lg">Tidak ada audit log ditemukan untuk brand ini</p>
            </div>
          ) : (
                    ...
                    <tr key={idx} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        <span className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs font-medium">
                          {getTableDisplayName(group.sourceTable)}
                        </span>
                      </td>
