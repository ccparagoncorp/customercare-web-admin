...
export default function EditCategory({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [categoryData, setCategoryData] = useState<Category | Subcategory | null>(null)
      ...
      <div className="space-y-8">
        ...
        <div className="bg-gradient-to-r from-[#03438f] to-[#012f65] rounded-2xl p-8 text-white">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold mb-2">{sections.category.form.title.edit}</h1>
              <p className="text-blue-100 text-lg">Edit {formData.type === 'category' ? 'kategori' : 'subkategori'}: {categoryData.name}</p>
            </div>
          </div>
        </div>
