  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(error.error || 'Error updating kategori SOP')
      }
    } catch (error) {
      console.error('Error updating kategori SOP:', error)
      alert('Error updating kategori SOP')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="space-y-8">
        ...
        <div className="bg-gradient-to-r from-[#03438f] to-[#012f65] rounded-2xl p-8 text-white">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold mb-2">Edit Kategori SOP</h1>
              <p className="text-blue-100 text-lg">Edit kategori SOP: {kategoriSOP.name}</p>
            </div>
          </div>
        </div>
