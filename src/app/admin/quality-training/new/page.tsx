...
export default function NewQualityTraining() {
  ...
  const [formData, setFormData] = useState({
    title: '',
    description: '',
    logos: '' // comma separated (will be managed via uploads)
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const err = await response.json(); alert(err.error || 'Error creating Quality & Training')
      }
    } catch (error) {
      console.error('Error creating Quality & Training:', error)
      alert('Error creating Quality & Training')
      ...
      <div className="space-y-8">
        <div className="bg-gradient-to-r from-[#03438f] to-[#012f65] rounded-2xl p-8 text-white">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold mb-2">Tambah Quality & Training</h1>
              <p className="text-blue-100 text-lg">Tambah entri Quality & Training baru</p>
            </div>
          </div>
        </div>
