...
export default function EditJenisQualityTraining() {
      ...
      if (res.ok) {
        ...
        const mappedDetails = (data.detailQualityTrainings || []).map((d: any) => ({
          ...
          subdetails: (d.subdetailQualityTrainings || []).map((s: any) => ({
            ...
            logos: Array.isArray(s.logos) ? s.logos.join(', ') : ''
          }))
        }))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/jenis-quality-training/${id}`, {
        method: 'PUT',
      ...
      if (response.ok) {
        ...
        const err = await response.json(); alert(err.error || 'Error updating Jenis')
      }
    } catch (error) {
      console.error('Error updating Jenis Quality & Training:', error)
      alert('Error updating Jenis')
      ...
      <div className="space-y-8">
        <div className="bg-gradient-to-r from-[#03438f] to-[#012f65] rounded-2xl p-8 text-white">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold mb-2">Edit Jenis Quality & Training</h1>
              <p className="text-blue-100 text-lg">Ubah jenis beserta detail dan subdetail</p>
            </div>
          </div>
        </div>
