...
export default function NewAnnouncement() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        alert('Announcement berhasil dibuat!')
        router.push('/admin/announcement')
      } else {
        const error = await response.json()
        alert(error.error || 'Error creating announcement')
      }
    } catch (error) {
      ...
      <div className="space-y-8">
        ...
        <div className="bg-gradient-to-r from-[#03438f] to-[#012f65] rounded-2xl p-8 text-white">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold mb-2">Tambah Announcement</h1>
              <p className="text-blue-100 text-lg">Buat announcement baru</p>
            </div>
          </div>
        </div>
